export const site = {
  name: "Manuel Figueira",
  initials: "MF",
  url: "https://manufigueira.pro",

  // TODO: replace — placeholder hero copy, pending final wording.
  role: "Full-Stack Software Engineer. Eight years building web and Web3 products.",
  subtext:
    "Skilled at delivering solutions while collaborating closely with product, design and engineering. Thrives in remote, distributed teams and brings a strong attention to detail to every project.",

  seoTitle: "Manuel Figueira — Full-Stack Software Engineer",
  seoDescription:
    "Full-Stack Software Engineer with eight years building web and Web3 products with React, Next.js, TypeScript and Node.js.",

  social: {
    github: "https://github.com/Mfigueira",
    linkedin: "https://www.linkedin.com/in/manuel-figueira/",
  },

  closingLine: "Open to engineering roles and solving interesting problems.",
} as const;
